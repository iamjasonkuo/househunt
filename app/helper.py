...
def generateMarker(array):
    markers = []
    for project in array:
        marker = Marker()
        try:
            marker.set_data(project)
            markers.append(marker)
        except:
            raise Error({'message': 'No valid marker data found %s' % project})
    return markers
...
class Marker(SourceData):
    ...
    def set_data(self, source_data):
        self.icon = 'http://maps.google.com/mapfiles/ms/icons/green-dot.png'
        self.lat = source_data.site.lat
        self.lng = source_data.site.lng
        self.infobox = '<b>score goes here</b>'
